    ...
    while step < end_step:
        ...
        def step_fn():
            ...
            if "gt_occ" in clean_batch:
                ...
                # Confidence must be calibrated against actual reconstruction
                # correctness, not the model's distance from its own 0.5
                # threshold (which merely measures entropy).
                geo_error = (occ_prob - losses["target_occupancy"]).abs().clamp(0.0, 1.0)
            else:
